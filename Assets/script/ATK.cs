...
public class ATK : MonoBehaviour
{
    ...
    public float speedProjectil = 1f;           // La vitesse de d�placement de notre projectile (valeur de base = 1)
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && !reloading && flag == 1)
        {
            shoot.Play();
            degats = PlayerPrefs.GetInt("degatDIST");
            ...
            projectilSave.GetComponent<Rigidbody2D>().velocity = direction * speedProjectil;                    // Et on fait avancer le projectile dans la direction qu'on a calcul� plut�t
